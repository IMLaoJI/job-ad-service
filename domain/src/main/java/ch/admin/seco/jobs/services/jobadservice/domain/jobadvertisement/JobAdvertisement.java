...
public class JobAdvertisement implements Aggregate<JobAdvertisement, JobAdvertisementId> {
    ...
    @AttributeOverrides({
            ...
            @AttributeOverride(name = "userDisplayName", column = @Column(name = "OWNER_USER_DISPLAY_NAME")),
    ...
    public void adjournPublication() {
    ...
    private ChangeLog applyUpdates(JobAdvertisementUpdater updater) {
        ...
        if (updater.hasAnyChangesIn(SECTION_PUBLIC_CONTACT) && hasChanged(jobContent.getPublicContact(), updater.getPublicContact())) {
            changeLog.add("publicContact", jobContent.getPublicContact(), updater.getPublicContact());
            jobContent.setPublicContact(updater.getPublicContact());
        }
