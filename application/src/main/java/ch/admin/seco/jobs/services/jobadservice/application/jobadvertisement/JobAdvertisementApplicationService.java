...
import static ch.admin.seco.jobs.services.jobadservice.application.common.logging.BusinessLogEventType.JOB_ADVERTISEMENT_ACCESS_EVENT;
...
public class JobAdvertisementApplicationService {
	...
	public JobAdvertisementId createFromWebForm(CreateJobAdvertisementDto createJobAdvertisementDto) {
		...
		LOG.debug("Create '{}'", createJobAdvertisementDto.getSingleJobDescription().getTitle());
	...
	public CreatedJobAdvertisementIdWithTokenDto createFromApi(CreateJobAdvertisementDto createJobAdvertisementDto) {
		...
		Condition.notNull(createJobAdvertisementDto, "CreateJobAdvertisementDto can't be null");
		String avamOccupationCode = createJobAdvertisementDto.getSingleOccupation().getAvamOccupationCode();
	...
	public JobAdvertisementId createFromExtern(CreateJobAdvertisementDto createJobAdvertisementFromExternalDto) {
		...
		LOG.debug("Create '{}'", createJobAdvertisementFromExternalDto.getSingleJobDescription());

        checkIfJobAdvertisementAlreadyExists(createJobAdvertisementFromExternalDto);

		Location location = toLocation(createJobAdvertisementFromExternalDto.getLocation());
		location = locationService.enrichCodes(location);

		List<Occupation> occupations = enrichAndToOccupations(createJobAdvertisementFromExternalDto.getOccupations());

		Company company = toCompany(createJobAdvertisementFromExternalDto.getCompany());
		JobContent jobContent = new JobContent.Builder()
                .setNumberOfJobs(createJobAdvertisementFromExternalDto.getNumberOfJobs())
				.setJobDescriptions(Collections.singletonList(
						buildJobDescription(createJobAdvertisementFromExternalDto.getSingleJobDescription().getTitle(), createJobAdvertisementFromExternalDto.getSingleJobDescription().getDescription())
				))
				...
				.setCompany(company)
				.setPublicContact(toPublicContact(createJobAdvertisementFromExternalDto.getPublicContact()))
				.setLanguageSkills(toLanguageSkills(createJobAdvertisementFromExternalDto.getLanguageSkills()))
				.build();

        LocalDate publicationStartDate = defaultIfNull(createJobAdvertisementFromExternalDto.getPublication().getStartDate(), TimeMachine.now().toLocalDate());
        LocalDate publicationEndDate = defaultIfNull(createJobAdvertisementFromExternalDto.getPublication().getEndDate(), publicationStartDate.plusDays(PUBLICATION_MAX_DAYS));

		final JobAdvertisementCreator creator = new JobAdvertisementCreator.Builder(currentUserContext.getAuditUser())
                .setFingerprint(createJobAdvertisementFromExternalDto.getFingerprint())
				.setJobContent(jobContent)
				.setContact(toContact(createJobAdvertisementFromExternalDto.getContact()))
				.setPublication(
						new Publication.Builder()
								...
								.setCompanyAnonymous(createJobAdvertisementFromExternalDto.getPublication().isCompanyAnonymous())
								.build()
				)
				.build();
	...
	public JobAdvertisementId updateFromExtern(JobAdvertisementId jobAdvertisementId, CreateJobAdvertisementDto createFromExternal) {
        ...
        Location location = toLocation(createFromExternal.getLocation());
        location = locationService.enrichCodes(location);
        LocalDate publicationStartDate = defaultIfNull(createFromExternal.getPublication().getStartDate(), TimeMachine.now().toLocalDate());
        LocalDate publicationEndDate = defaultIfNull(createFromExternal.getPublication().getEndDate(), publicationStartDate.plusDays(PUBLICATION_MAX_DAYS));

		JobAdvertisementUpdater updater = new JobAdvertisementUpdater.Builder(currentUserContext.getAuditUser())
				.setJobDescription(createFromExternal.getSingleJobDescription().getTitle(), createFromExternal.getSingleJobDescription().getDescription())
                ...
                .setPublication(new Publication.Builder()
                        ...
                        .setPublicDisplay(true)
                        .setRestrictedDisplay(true)
                        .setCompanyAnonymous(jobAdvertisement.getPublication().isCompanyAnonymous())
                        .build())
                .setEmployment(toEmployment(createFromExternal.getEmployment()))
                .setLocation(location)
                .setContact(toContact(createFromExternal.getContact()))
                .setX28OccupationCodes(createFromExternal.getProfessionCodes())
				.build();
	...
	public JobAdvertisementDto getById(JobAdvertisementId jobAdvertisementId) throws AggregateNotFoundException {
        ...
        BusinessLogData logData = BusinessLogData.of(JOB_ADVERTISEMENT_ACCESS_EVENT)
	...
	private JobAdvertisementUpdater prepareUpdaterFromAvam(CreateJobAdvertisementDto createJobAdvertisement) {
		...
		return new JobAdvertisementUpdater.Builder(currentUserContext.getAuditUser())
				...
				.setJobDescription(createJobAdvertisement.getSingleJobDescription().getTitle(), createJobAdvertisement.getSingleJobDescription().getDescription())
	...
	private void checkIfJobAdvertisementAlreadyExists(CreateJobAdvertisementDto createJobAdvertisementFromAvamDto) {
		...
		jobAdvertisement = jobAdvertisementRepository.findByFingerprint(createJobAdvertisementFromAvamDto.getFingerprint());
		jobAdvertisement.ifPresent(jobAd -> {
